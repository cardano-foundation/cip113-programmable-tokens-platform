...
# Read once, up front: --regenerate needs it to protect a rebuilt blueprint from the
# rsync, and the tree diff below needs it to skip that file.
BLUEPRINT_SOURCE=$(python3 -c "import json;print(json.load(open('$PIN')).get('blueprint_source','verbatim'))")
BLUEPRINT_EXCLUDE=()
if [[ "$BLUEPRINT_SOURCE" == "rebuilt-from-vendored-source" ]]; then
  BLUEPRINT_EXCLUDE=(--exclude=plutus.json)
fi
...
if [[ $REGENERATE -eq 1 ]]; then
  ...
  # When the blueprint is a rebuild, it is protected here too: rsync must not put
  # upstream's stale plutus.json back over it, or --regenerate would silently undo the
  # exception and then record the stale bytes in the manifest.
  RSYNC_BLUEPRINT_EXCLUDE=()
  if [[ "$BLUEPRINT_SOURCE" == "rebuilt-from-vendored-source" ]]; then
    RSYNC_BLUEPRINT_EXCLUDE=(--exclude='plutus.json')
  fi
  rsync -a --delete \
...
# plutus.json may be REBUILT rather than vendored verbatim, when upstream's committed
# blueprint does not reproduce from its own source (see blueprint_reproduced_note in
# UPSTREAM_PIN.json). The exception is narrow and self-expiring: both the upstream
# committed bytes and our rebuilt bytes are pinned by sha256 just below, so the moment
# upstream regenerates its blueprint this script fails and tells us to drop the exception
# rather than silently carrying a blind spot forever.
...
# 1b. If the blueprint is a rebuild, BOTH sides are pinned by hash.
#
#     - the vendored (rebuilt) blueprint must be exactly the bytes the pin recorded, so a
#       local edit to plutus.json is still caught even though the tree diff skipped it;
#     - upstream's COMMITTED blueprint must still be the stale one we diagnosed. If it
#       changes, upstream has (probably) regenerated it, and the right move is to re-vendor
#       verbatim and delete this exception -- not to keep rebuilding on top of a moving base.
if [[ "$BLUEPRINT_SOURCE" == "rebuilt-from-vendored-source" ]]; then
  WANT_LOCAL=$(python3 -c "import json;print(json.load(open('$PIN'))['blueprint_rebuilt_sha256'])")
  WANT_UP=$(python3 -c "import json;print(json.load(open('$PIN'))['blueprint_upstream_committed_sha256'])")
  GOT_LOCAL=$(shasum -a 256 "$HERE/plutus.json" | awk '{print $1}')
  GOT_UP=$(shasum -a 256 "$UP/plutus.json" | awk '{print $1}')
  if [[ "$GOT_LOCAL" != "$WANT_LOCAL" ]]; then
    echo "FAIL: the vendored plutus.json is not the rebuild the pin recorded."
    echo "      expected $WANT_LOCAL"
    echo "      got      $GOT_LOCAL"
    echo "      Rebuild it from the vendored source with the pinned aiken, or re-vendor."
    exit 1
  fi
  if [[ "$GOT_UP" != "$WANT_UP" ]]; then
    echo "FAIL: upstream's committed plutus.json changed ($GOT_UP)."
    echo "      The pin carries a 'rebuilt-from-vendored-source' exception because upstream's"
    echo "      blueprint was STALE at $SHA. If upstream has regenerated it, that exception is"
    echo "      obsolete: check whether the committed blueprint now reproduces, and if it does,"
    echo "      drop blueprint_source/blueprint_*_sha256 from UPSTREAM_PIN.json and re-run"
    echo "      ./verify-upstream-pin.sh --regenerate to vendor it verbatim again."
    exit 1
  fi
  echo "NOTE: plutus.json is a REBUILD of the vendored source, not upstream's committed"
  echo "      blueprint, which is stale for global_state_spend. Both sides pinned by sha256."
fi
...
if [[ "$STATE" == "dirty-worktree" ]]; then
  echo "OK: vendored rwa-token tree matches its manifest (dirty-worktree pin,"
  echo "    base $SHA — no upstream revision was fetched; see UPSTREAM_PIN.json)."
else
  echo "OK: vendored rwa-token tree is verbatim $REPO @ $SHA."
fi
