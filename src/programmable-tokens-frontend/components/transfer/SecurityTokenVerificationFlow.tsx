...
/**
 * Tagged union describing how the sender will be authenticated to the security-token
 * transfer validator. Routed by {@code TransferModal} to the right request fields.
 */
export type SecurityTokenSenderProofPayload =
...
export interface SecurityTokenVerificationResult {
  sender: SecurityTokenSenderProofPayload;
...
interface Props {
  ...
  onComplete: (result: SecurityTokenVerificationResult) => void;
  onBack: () => void;
}
...
type Phase =
  ...
  | { kind: "running-keri" }
  | { kind: "probing-receiver"; sender: SecurityTokenSenderProofPayload }
  | { kind: "receiver-not-verified"; sender: SecurityTokenSenderProofPayload; mode: "not-verified" | "expired" | "publish-pending" }
  | { kind: "error"; message: string };

export function SecurityTokenVerificationFlow({
  policyId,
  senderAddress,
  recipientAddress,
  requiresReceiverKyc,
  ...
  const senderStatus = useSecurityTokenMembershipStatus(policyId, senderAddress);
