 ...
 */
export function participantColour(keyHash) {
 ...
 */
export function confusablePairs(keyHashes, minSeparation = 25) {
    const pairs = [];
    for (let i = 0; i < keyHashes.length; i++) {
        for (let j = i + 1; j < keyHashes.length; j++) {
            ...
            if (Math.min(d, 360 - d) < minSeparation)
                pairs.push([keyHashes[i], keyHashes[j]]);
        }
    }
    return pairs;
}
