/**
 * Shared security-token allowlist membership state. 60s TTL with stale-while-revalidate
 ...
 *
 * Parallel to {@link useMpfMembershipStatus} but bound to the security-token API
...
import { getSecurityTokenInclusionProof } from "@/lib/api/security-token";
import { ApiException } from "@/types/api";

export type SecurityTokenMembershipStatus =
  | { kind: "loading" }
  | { kind: "not-verified" }
...
interface CacheEntry {
  status: SecurityTokenMembershipStatus;
  fetchedAt: number;
}
...
const inFlight = new Map<string, Promise<SecurityTokenMembershipStatus>>();

function cacheKey(policyId: string, walletAddress: string | null) {
  return `${policyId}:${walletAddress ?? ""}`;
}

async function probe(policyId: string, walletAddress: string): Promise<SecurityTokenMembershipStatus> {
  ...
  try {
    const proof = await getSecurityTokenInclusionProof(policyId, memberPkh);
    return {
      kind: "verified",
...
function getOrFetch(policyId: string, walletAddress: string): Promise<SecurityTokenMembershipStatus> {
...
interface UseSecurityTokenMembershipStatusResult {
  status: SecurityTokenMembershipStatus;
  refresh: () => void;
}

export function useSecurityTokenMembershipStatus(
  policyId: string | null,
  walletAddress: string | null,
): UseSecurityTokenMembershipStatusResult {
  const [status, setStatus] = useState<SecurityTokenMembershipStatus>({ kind: "loading" });
