 ...
 *
 * A module id is a key: it is written into the database, sent in registration callbacks, and
 ...
 * the Swiss framework the security module implements — a matched taxonomy rather than one
...
export type ModuleKind = 'stable' | 'security' | 'template' | 'unknown';
...
export interface ModuleLabel {
  ...
  kind: ModuleKind;
  /** One line for the detail panel. Says what the module actually does. */
  blurb: string;
}
 ...
 *
 * A new module should look unlabelled, not broken — and the raw id is still the most useful
 * thing anyone can be told about it.
 */
export function moduleLabel(id: string | null | undefined): ModuleLabel {
  if (!id) {
    return {
      label: 'Unregistered',
      kind: 'unknown',
      blurb: 'This token is in the registry but the backend has no module recorded for it.',
    };
  }
  return (
    LABELS[id] ?? { label: id, kind: 'unknown', blurb: `Module "${id}" has no display label yet.` }
  );
}

/** Ids offered as filter chips, in the order they are offered. */
export const LABELLED_MODULES = ['freeze-and-seize', 'rwa-token', 'dummy'] as const;
