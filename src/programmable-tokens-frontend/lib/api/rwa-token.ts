...
export const listRwaTokens = () =>
  apiGet<RwaTokenSummary[]>(`/rwa-token/tokens`);

/** On-chain GS datum for a registered rwa-token policy. The mint form
...
export const getRwaTokenGlobalState = (policyId: string) =>
  apiGet<RwaTokenGlobalState>(`/rwa-token/${policyId}/global-state`);

export interface RwaTokenAdminInfo {
  adminPkh: string;
  adminAddress: string;
}

/** Backend's admin signing key PKH. Must be used as issuerAdminPkh when registering
 *  a rwa-token so the backend can autonomously sign UpdateMemberRootHash. */
export const getRwaTokenAdminPkh = () =>
  apiGet<RwaTokenAdminInfo>(`/rwa-token/admin-pkh`);

// ── Genesis init ────────────────────────────────────────────────────────────

/** Request body for {@link initRwaTokenGlobalState}. Mirrors the backend's
 *  {@code RwaTokenRegisterRequest} for the fields the init flow needs. */
export interface RwaTokenInitRequest {
   ...
   *  on chain. Only honoured by {@link buildRwaTokenChain}. */
...
export interface RwaTokenInitResponse {
...
export interface GsChangeSpec {
   ...
   *  (RwaTokenController: `change.get("requiresSenderKycEnabled")`); it was
...
export const buildGlobalStateUpdateChain = (
  ...
  apiPost<{ feePayerAddress: string; changes: GsChangeSpec[] }, { unsignedCborTxs: string[] }>(
    `/rwa-token/${policyId}/global-state/update-chain`,
...
export const buildRegisterTransferLogicTx = (policyId: string, feePayerAddress: string) =>
  apiPost<{ feePayerAddress: string }, { unsignedCborTx: string }>(
    `/rwa-token/${policyId}/register-transfer-logic`,
    { feePayerAddress },
  );
...
export const listDenylist = (policyId: string) =>
  apiGet<DenylistEntry[]>(`/rwa-token/${policyId}/denylist`);

export const addDenylistEntry = (
  policyId: string,
  body: { memberPkh: string; reason?: string; addedByPowerUserPkh?: string },
) =>
  apiPost<typeof body, DenylistEntry>(`/rwa-token/${policyId}/denylist`, body);

export const removeDenylistEntry = (policyId: string, memberPkh: string) =>
  apiDelete<void>(`/rwa-token/${policyId}/denylist/${memberPkh}`);

// ── Power users ─────────────────────────────────────────────────────────────

/** Capability bitfield bits — MUST mirror {@code RwaTokenPowerUserCapability}
...
export const listPowerUsers = (policyId: string) =>
  apiGet<PowerUser[]>(`/rwa-token/${policyId}/power-users`);

export const addPowerUser = (
  policyId: string,
  body: { powerUserPkh: string; capabilities: number; label?: string },
) =>
  apiPost<typeof body, PowerUser>(`/rwa-token/${policyId}/power-users`, body);

export const removePowerUser = (policyId: string, powerUserPkh: string) =>
  apiDelete<void>(`/rwa-token/${policyId}/power-users/${powerUserPkh}`);
...
export const addPowerUserOnChain = (
  ...
  apiPost<typeof body, { unsignedCborTx: string }>(
    `/rwa-token/${policyId}/power-users/on-chain`,
    body,
  );
