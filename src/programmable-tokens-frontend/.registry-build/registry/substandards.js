...
const LABELS = {
    ...
    'rwa-token': {
        label: 'CMTA · eWpG',
        kind: 'security',
        blurb: 'Tokenised securities under the Swiss CMTA framework and the German Electronic Securities ' +
 ...
 */
export function substandardLabel(id) {
    if (!id) {
        return {
            label: 'Unregistered',
            kind: 'unknown',
    ...
    return (LABELS[id] ?? { label: id, kind: 'unknown', blurb: `Substandard "${id}" has no display label yet.` });
}
/** Ids offered as filter chips, in the order they are offered. */
export const LABELLED_SUBSTANDARDS = ['freeze-and-seize', 'rwa-token', 'dummy'];
