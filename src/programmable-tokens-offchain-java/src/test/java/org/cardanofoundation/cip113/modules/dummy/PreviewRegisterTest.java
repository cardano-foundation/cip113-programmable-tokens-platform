package org.cardanofoundation.cip113.modules.dummy;
...
/**
 * End-to-end CIP-113 registration + subsequent mint for the {@code dummy} module.
 *
...
public class PreviewRegisterTest extends AbstractPreviewTest {
    ...
    @BeforeEach
    public void init() {
        moduleService = new ModuleService(OBJECT_MAPPER);
        moduleService.init();
    ...
    public void test() throws Exception {
        ...
        var issuerContractOpt = moduleService.getModuleValidator(MODULE_NAME, "transfer.issue.withdraw");
        var issuerContract = issuerContractOpt.get();

        var moduleIssueContract = PlutusBlueprintUtil.getPlutusScriptFromCompiledCode(issuerContract.scriptBytes(), PlutusVersion.v3);
        ...
        var transferContractOpt = moduleService.getModuleValidator(MODULE_NAME, "transfer.transfer.withdraw");
        var transferContract = transferContractOpt.get();

        var moduleTransferContract = PlutusBlueprintUtil.getPlutusScriptFromCompiledCode(transferContract.scriptBytes(), PlutusVersion.v3);
        ...
        // force-transfer validator (src/modules/dummy/validators/transfer.ak has only
        ...
        // The module's withdraw-0 reward accounts must exist before registry_mint's
        // "module authorises this registration" check can be satisfied. Idempotent:
        ...
        registerStakeAddressIfNeeded(issuerAccount, moduleIssueAddress.getAddress(), moduleTransferAddress.getAddress());
    ...
    public void mint() throws Exception {
        ...
        var moduleIssueContract = PlutusBlueprintUtil.getPlutusScriptFromCompiledCode(
                moduleService.getModuleValidator(MODULE_NAME, "transfer.issue.withdraw").orElseThrow().scriptBytes(),
                PlutusVersion.v3);
        var moduleIssueAddress = AddressProvider.getRewardAddress(moduleIssueContract, network);
