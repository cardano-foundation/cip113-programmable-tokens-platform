...
import org.cardanofoundation.cip113.entity.RwaTokenPowerUserCapability;
 ...
 *  isolated from {@code KycExtendedModuleHandler} and {@code MpfTreeService} so
 *  the older KYC modules can be removed without breaking it.
 *
 ...
 *        {@code RwaTokenAllowlistService}; the off-chain trie must match byte for
...
@Slf4j
public class RwaTokenModuleHandler
        implements ModuleHandler,
                   BasicOperations<RwaTokenRegisterRequest>,
                   org.cardanofoundation.cip113.service.module.capabilities.BlacklistManageable,
                   // Seizure is a first-class capability of this module, not an
                   ...
                   // request with "Module 'rwa-token' does not support seize
                   // operations" while the on-chain path was sitting there implemented.
                   org.cardanofoundation.cip113.service.module.capabilities.Seizeable {
    ...
    private final RwaTokenScriptBuilderService scriptBuilder;
     ...
     *  the first mint, which is where this module's reference token is created. */
    ...
    @Setter
    private RwaTokenContext context;
    ...
    public TransactionContext<List<String>> buildPreRegistrationTransaction(
        ...
        // rwa-token bundles the module stake-credential registration into
        // the registration tx (see buildRegistrationTransaction). The wizard
        // doesn't call a separate pre-registration step for this module.
        return TransactionContext.ok(null, List.of());
    }

    /** Registration tx. Mints the rwa-token policy under prog-logic-base,
     *  registers it in the CIP-113 directory with the parameterised
     *  {@code transfer_logic_script.withdraw} hash, and persists the
     *  {@link RwaTokenRegistrationEntity} row.
     *
     ...
     *
     *  <p>Reference: mirrors {@code KycExtendedModuleHandler.buildRegistrationTransaction}
    ...
    public TransactionContext<RegistrationResult> buildRegistrationTransaction(
        ...
        try {
            ...
            // RwaTokenRegistrationEntity row keyed on the prog-token policy
            // id and stored the (gs, pu, dl) policies + securityAssetNameHex +
            // bootstrap input. TokenOperationsService loaded all of that into
            // the RwaTokenContext for us.
            if (context == null) {
                return TransactionContext.typedError("rwa-token context not set — genesis init must run first");
            }
            ...
            if (gsPolicy == null || puPolicy == null || securityAssetNameHex == null || adminPkhHex == null) {
                return TransactionContext.typedError(
                        "rwa-token context is incomplete — re-run the genesis init step (gsPolicy/puPolicy/assetName/adminPkh missing)");
            }
            ...
            RwaTokenScriptBuilderService.RwaTokenScripts scripts = scriptBuilder.resolveScripts(
            ...
            RegistryNode directorySpendDatum = existingNode.toBuilder()
                    ...
                    .build();
            // third_party_transfer_logic_script (index 4) = the module's OWN
            ...
            // permission by default; rwa-token declares no unfracking hook validator.
            ...
            // 6. Recipient = the user's wallet (where the first minted RWA tokens go).
            ...
            if (!willMint && Cip68.hasLabel(securityAssetNameHex)) {
                RwaTokenRegistrationEntity metaRow = registrationRepository
            ...
            Utxo gsUtxoForRegistration = chainedGsUtxoOverride != null
                    ...
                    : utxoProvider.findUtxoByAsset(gsPolicy,
                            RwaTokenScriptBuilderService.GLOBAL_STATE_ASSET_NAME_HEX).orElse(null);
            ...
            if (willMint) {
                ...
                RwaTokenRegistrationEntity reg = registrationRepository
            ...
            // Compute it only after optional module reference scripts are known.
            if (willMint) {
            ...
            int gsRefIdxForRegistration = willMint
                    ? 0
                    : regResolved.referenceInputIndex(txInputOf(gsUtxoForRegistration));
            if (willMint) {
            ...
            if (willMint) {
                ...
                if (regReferenceToken != null) {
                    RwaTokenRegistrationEntity refRow = registrationRepository
                            .findByProgrammableTokenPolicyId(progTokenPolicyId).orElse(null);
                    if (refRow != null) {
                ...
                log.info("rwa-token registration carries a first mint: policy={} qty={} "
                         + "(mintable_amount {} → {})",
            ...
            // rwa-token module. (The RwaTokenRegistrationEntity
            ...
            log.error("rwa-token registration failed", e);
    ...
    /** Mint additional RWA tokens on a registered policy.
     *
     ...
     *            issuance contract uses it to find the registered module)
     ...
     *
     *    Mints:
     *      `quantity` RWA tokens under issuance contract
     *
     *    Outputs:
        ...
        try {
            ...
            RwaTokenRegistrationEntity reg = registrationRepository
                    ...
                    .orElseThrow(() -> new BuildPreconditionException(
                            "rwa-token registration not found for policy " + request.tokenPolicyId()));
            ...
            PlutusData authorityRedeemer = ConstrPlutusData.of(AUTHORITY_MINT_BURN,
                    // registry_node_ref_input_index is GONE as of the 2026-08-21
                    // upstream: this validator no longer reads the CIP-113 registry
                    // node at all. The base layer already authenticates whichever node
                    // invoked the withdraw-0, and registry field 4 is a FREE field a
                    // decoy node could name too, so the old identity assertion was
                    // never load-bearing. The issuance policy is a compile-time
                    // parameter now. (The directory entry stays in the reference-input
                    // set — CIP-113's own prog-logic-global redeemer still indexes it.)
                    BigIntPlutusData.of(BigInteger.valueOf(gsInputIdx)),
                    BigIntPlutusData.of(BigInteger.valueOf(puNodeRefIdx)),
                    BigIntPlutusData.of(mintQuantity),
            ...
            log.info("rwa-token mint: policy={} qty={} (mintable_amount {} → {})",
            ...
            log.error("rwa-token mint failed for policy={}", request.tokenPolicyId(), e);
    ...
    public TransactionContext<Void> buildTransferTransaction(
        ...
        try {
            ...
            if (regOpt.isEmpty()) {
                return TransactionContext.typedError(
                        "rwa-token registration not found for policy " + policyId);
            }
            ...
            // RwaTokenRegistrationEntity (the DB cache is set at registration
            ...
            // logic module script plus the protocol-level prog-logic-base/global.
            ...
            if (!transferLogicCredRegistered) {
                return TransactionContext.typedError(
                        ...
                        + "Call POST /rwa-token/" + policyId
                        + "/register-transfer-logic first (one-time admin action), "
                        + "then retry the transfer.");
            }

            // A transfer loads the `transfer` delegate and neither of the other two:
            // programmable_logic_base dispatches to exactly one delegate per input, so the
            // seize and unfracking scripts are never paid for here.
            PlutusScript coreTransfer = protocolScriptBuilderService
                    .getParameterizedTransferScript(protocolParams);
            ...
            // Declared together, before any index is taken. The module's transfer-logic
            // withdrawal belongs in here too: wdrl_idx is a position over the WHOLE withdrawal
            // map, so a layout that saw only the framework's would be right in isolation and
            // wrong in the transaction.
            Credential transferLogicCredential = Credential.fromScript(transferLogicScript.getScriptHash());
            ...
            tx = tx
                    ...
                    // NOT attachRewardValidator(coreTransfer): the transfer delegate is READ from
                    // the reference script the deployment published (tiTransferRef, in
                    // refInputsSorted above), exactly as programmable_logic_base is. Attaching it
                    // as well would put the same script in the witness set AND in a reference
                    // input for the same credential -- rejected phase-1 as
                    // ExtraneousScriptWitnessesUTXOW, and on the hot path, where the whole point
                    // of referencing it is that every transfer would otherwise pay for its bytes.
            ...
            log.info("rwa-token transfer: policy={} amount={} {} → {}",
                    policyId, amountToTransfer, senderAddrBech32, recipientAddress.getAddress());
            return TransactionContext.ok(transaction.serializeToHex());
        } catch (Exception e) {
            log.error("rwa-token transfer failed for unit={}", request.unit(), e);
     ...
     *  with initial datums. Also writes the {@link RwaTokenRegistrationEntity}
     *  row and, when {@code bootstrapPowerUserPkh} is set on the request, seeds the
     *  off-chain {@code RwaTokenPowerUserEntity} row so the admin sees themselves
    ...
    public TransactionContext<RegistrationResult> buildGlobalStateInitTransaction(
        ...
        try {
            ...
            // modules, so none of them may claim (222). See Cip68.userTokenLabel.
            if (request.getCip68Metadata() != null) {
                ...
                log.info("rwa-token genesis: CIP-68 enabled, security asset name {} -> {} "
            ...
            if (request.isSeedRecipientInAllowlistAtGenesis() && !seedApplies) {
                log.info("rwa-token genesis: ignoring seedRecipientInAllowlistAtGenesis — "
            ...
            if (seedApplies) {
                ...
                log.warn("rwa-token genesis: OPT-IN allowlist seed enabled — enrolling stake "
            ...
            Asset gsNft = Asset.builder()
                    .name("0x" + RwaTokenScriptBuilderService.GLOBAL_STATE_ASSET_NAME_HEX)
                    .value(BigInteger.ONE).build();
            ...
            if (!powerUserRepository.existsByProgrammableTokenPolicyIdAndPowerUserPkh(
                ...
                powerUserRepository.save(RwaTokenPowerUserEntity.builder()
                        .programmableTokenPolicyId(issuancePolicyId)
            ...
            log.error("rwa-token global-state init failed", e);
    ...
    private static ConstrPlutusData powerUserData(byte[] pkh, int capabilities) {
        return ConstrPlutusData.of(0,
                ...
                plutusBool((capabilities & RwaTokenPowerUserCapability.ADMIN.bit()) != 0),
                plutusBool((capabilities & RwaTokenPowerUserCapability.MINTER.bit()) != 0),
                plutusBool((capabilities & RwaTokenPowerUserCapability.BURNER.bit()) != 0),
                plutusBool((capabilities & RwaTokenPowerUserCapability.PAUSER.bit()) != 0),
                plutusBool((capabilities & RwaTokenPowerUserCapability.FORCE_TRANSFER.bit()) != 0));
    }
    ...
    public TransactionContext<Void> buildAddPowerUserTransaction(
        ...
        try {
            Optional<RwaTokenRegistrationEntity> regOpt = registrationRepository.findByProgrammableTokenPolicyId(policyId);
            if (regOpt.isEmpty()) {
                return TransactionContext.typedError("rwa-token registration not found for policy " + policyId);
            }
            RwaTokenRegistrationEntity reg = regOpt.get();
            ...
            log.error("rwa-token AddPowerUser failed for policy={} pkh={}",
    ...
    public TransactionContext<TransactionContext.MintingResult> buildBlacklistInitTransaction(
            ...
            ProtocolBootstrapParams protocolParams) {
        return TransactionContext.typedError(
                "rwa-token denylist is initialised at genesis (buildGlobalStateInitTransaction). "
                + "Calling /compliance/blacklist/init for a rwa-token is not supported.");
    }

    @Override
    public TransactionContext<Void> buildAddToBlacklistTransaction(
            org.cardanofoundation.cip113.service.module.capabilities.BlacklistManageable.AddToBlacklistRequest request,
            ProtocolBootstrapParams protocolParams) {
    ...
    public TransactionContext<Void> buildRemoveFromBlacklistTransaction(
            org.cardanofoundation.cip113.service.module.capabilities.BlacklistManageable.RemoveFromBlacklistRequest request,
            ProtocolBootstrapParams protocolParams) {
        return buildDenylistMutation(request.tokenPolicyId(),
    ...
    private TransactionContext<Void> buildDenylistMutation(
        ...
        try {
            ...
            // power-user spend script first — see RwaTokenScriptBuilderService.
            ...
            boolean signerIsAdmin = powerUserRepository
                    ...
                    .map(pu -> RwaTokenPowerUserCapability.ADMIN.granted(pu.getCapabilities()))
                    .orElse(false);
            if (!signerIsAdmin) {
                return TransactionContext.typedError(
     ...
     * why the BURN — the only rwa-token path that spends a programmable-token UTxO,
     * and so the only one that runs PLB — fails on chain while evaluating perfectly
     * offline, and why {@code rwaTokenBurnFitsUnderMaxTxSize} flakes about one run in
    ...
    public TransactionContext<PublishedRefScripts> buildPublishScriptsTransaction(
        ...
        try {
            ...
            log.error("rwa-token publish-scripts failed", e);
     ...
     *  carries {@link RwaTokenRegisterRequest#getInitialMintQuantity()}, in which
    ...
    public TransactionContext<ChainBuildResult> buildFullRegistrationChain(
        ...
        try {
            ...
            // the RwaTokenRegistrationEntity row keyed on the prog-token
            // policy id. The row is what the next phase's script construction
            // and the controller's loadRwaTokenContext rely on.
            ...
            // empty RwaTokenContext. buildRegistrationTransaction reads
            // context fields, so populate from the row we just persisted before
            // continuing into phase 3.
            this.context = RwaTokenContext.builder()
            ...
            // checked here", and the module's own RegisterToken branch validates
    ...
    /** Burn RWA tokens — the mirror image of {@link #buildMintTransaction}.
     *
     *  <h3>Tx shape</h3>
     *  <pre>
     *    Inputs:
     ...
     *
     *    Mints:
     *      -quantity RWA tokens under issuance contract (negative = burn)
     *
     *    Outputs:
    ...
    public TransactionContext<Void> buildBurnTransaction(
        ...
        try {
            ...
            // burns the entire UTxO because of its own module validator
            ...
            TransactionInput progBaseRefInput = TransactionInput.builder()
                    ...
                    .build();
            // A burn or seizure is an ADMINISTRATIVE action, so the delegate it loads is
            // `third_party` -- a different validator with its own published reference script,
            // not a redeemer constructor on the transfer one.
            ...
            // Taken from `s` rather than re-resolving: MintLikeScripts.mintingLogic IS
            // RwaTokenScripts.mintingLogicProxy (see buildMintLikeScripts), and resolving the set
            // twice invites the two copies to drift if either call site is ever changed.
            // Resolved here rather than at its first use further down, because the layout needs
            // it: a withdrawal that is emitted but not declared shifts every index after it.
            ...
            CoreLayout layout = CoreLayout.builder()
                    ...
                    .build();
            List<TransactionInput> refInputsSorted = layout.referenceInputs();
            int paramsIdx = layout.referenceInputIndex(txInputOf(protocolParamsUtxo));
            int directoryRefIdx = layout.referenceInputIndex(txInputOf(directoryEntry));
            int puNodeRefIdx = layout.referenceInputIndex(txInputOf(puNode));
            int denylistRefIdxForBurn = layout.referenceInputIndex(txInputOf(denylistCoveringNodeForBurn));
            ...
            // already needed satisfied it for free. Slot 4 now holds the module's
            ...
            // Slot 4 now holds the module's REAL
            ...
            // All FOUR withdrawals, added in LEDGER order. This is the widest withdrawal set in
            // the codebase, which is exactly where insertion order is most likely to differ from
            // the canonical one -- and a withdrawal's redeemer is matched to its entry by
            // position in that canonical map, not by the order it was added.
            for (CoreWithdrawal w : layout.inWithdrawalOrder(
                    List.of(new CoreWithdrawal(Credential.fromScript(s.mintingLogic().getScriptHash()),
                                    s.mintingLogicRewardAddress().getAddress(), proxyRedeemer),
                            new CoreWithdrawal(Credential.fromScript(s.mintingAuthority().getScriptHash()),
            ...
            log.info("rwa-token burn: policy={} qty={} (mintable_amount {} → {})",
                    request.tokenPolicyId(), burnQuantity, currentMintable, newMintable);
            return TransactionContext.ok(transaction.serializeToHex());
        } catch (BuildPreconditionException bpe) {
            return TransactionContext.typedError(bpe.getMessage());
        } catch (Exception e) {
            log.error("rwa-token burn failed for policy={}", request.tokenPolicyId(), e);
     ...
     *  nothing needed registering. Commit {@code 0ec401a} put the module's real
    ...
    private TransactionContext<Transaction> buildRegisterScriptStakeCredentialTransaction(
        ...
        try {
            ...
            log.info("rwa-token register-{}: policy={} stake={} size={} bytes",
            ...
            log.error("rwa-token register-{} failed for policy={}", label, policyId, e);
            return TransactionContext.typedError(
                    "register-" + label + " failed: " + e.getMessage());
        }
    }

    // ── Used by RwaTokenRootSyncJob ─────────────────────────────────────
    ...
    public TransactionContext<Void> buildUpdateMemberRootHashTransaction(
        ...
        try {
            ...
            log.info("rwa-token UpdateMemberRootHash: policy={} new_root={}",
    ...
    private TransactionContext<Void> buildSingleGsUpdateTx(
            RwaTokenRegistrationEntity reg,
     ...
     *
     *  <p>Looks up the global-state policy id on the RwaTokenRegistrationEntity
    ...
    private MintLikeScripts buildMintLikeScripts(
            RwaTokenRegistrationEntity reg, ProtocolBootstrapParams protocolParams)
            throws com.bloxbean.cardano.client.exception.CborSerializationException {
        RwaTokenScriptBuilderService.RwaTokenScripts scripts = resolveScripts(reg, protocolParams);
    ...
    private RwaTokenScriptBuilderService.RwaTokenScripts resolveScripts(
            RwaTokenRegistrationEntity reg, ProtocolBootstrapParams protocolParams) {
    ...
    private Utxo findGsUtxo(RwaTokenRegistrationEntity reg) {
        Utxo gsUtxo = utxoProvider.findUtxoByAsset(
                reg.getGlobalStatePolicyId(),
                RwaTokenScriptBuilderService.GLOBAL_STATE_ASSET_NAME_HEX
        ).orElseThrow(() -> new BuildPreconditionException(
    ...
    private Utxo findPuNode(RwaTokenRegistrationEntity reg, byte[] signerPkh, String role) {
    ...
    public TransactionContext<Void> buildUpgradeRegistryNodeTransaction(
        ...
        try {
            ...
            RwaTokenScriptBuilderService.RwaTokenScripts scripts =
                    resolveScripts(reg, protocolParams);
            ...
            log.info("rwa-token registry-node upgrade built: policy={} transfer {} -> {} "
                     + "thirdParty {} -> {}", policyId,
    ...
    /**
     * Seize the RWA token from a holder and place it at a destination.
     *
     * <p>This is the module's regulatory force-transfer path, and it is the same
     ...
     *   <li>at least one SPENT input carries the RWA token, so a "seizure" cannot be
        ...
        try {
            ...
            if (Integer.valueOf(Cip68.LABEL_REFERENCE).equals(Cip68.readLabel(assetNameHex))) {
                return TransactionContext.typedError(
                        ...
                        + "this module is parameterised by the USER token's asset name, so a "
            ...
            if (reg == null) {
                return TransactionContext.typedError(
                        "no rwa-token registration for policy " + policyId);
            }
            ...
            CoreLayout layout = CoreLayout.builder()
                    ...
                    .referenceInput(txInputOf(protocolParamsUtxo))
                    .referenceInput(txInputOf(issuanceUtxo))
                    .referenceInput(txInputOf(gsUtxo))
                    .referenceInput(txInputOf(denylistCoveringNode))
                    .referenceInput(progBaseRefInput)
                    .referenceInput(coreThirdPartyRefInput)
            ...
            int directoryRefIdx = layout.referenceInputIndex(txInputOf(directoryEntry));
            int puNodeRefIdx = layout.referenceInputIndex(txInputOf(puNode));
            int gsRefIdx = layout.referenceInputIndex(txInputOf(gsUtxo));
            int denylistRefIdx = layout.referenceInputIndex(txInputOf(denylistCoveringNode));
            ...
            // stake credentials from outputs carrying the RWA token, in output order;
            ...
            // the same shape the freeze-and-seize module uses.
            ...
            log.info("rwa-token seize built: policy={} qty={} from={}#{} to={} seizer={}",
            ...
            log.error("rwa-token seize failed for unit={}", request.unit(), e);
