...
/** Runtime context for a security-token handler instance. Carries the policy ids
 *  the ported BaFin validators are parameterised on, plus the MPF root tracking
 *  fields used by the autonomous publisher. */
@Getter
@Builder
@ToString
public class SecurityTokenContext implements SubstandardContext {
    ...
    /** Asset name of the security token, hex-encoded. Baked into several validators
    ...
    public static SecurityTokenContext emptyContext() {
        return SecurityTokenContext.builder().build();
    }
}
