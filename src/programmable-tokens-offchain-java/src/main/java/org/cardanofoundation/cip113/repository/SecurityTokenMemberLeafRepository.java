...
@Repository
public interface SecurityTokenMemberLeafRepository
        extends JpaRepository<SecurityTokenMemberLeafEntity, Long> {

    List<SecurityTokenMemberLeafEntity> findByProgrammableTokenPolicyId(String policyId);

    /** Only the leaves that have been included in a successful on-chain root publish. */
    @Query("SELECT e FROM SecurityTokenMemberLeafEntity e " +
    ...
    @Modifying
    @Query("UPDATE SecurityTokenMemberLeafEntity e SET e.publishedAt = :publishedAt " +
    ...
    List<SecurityTokenMemberLeafEntity> findExpired(
