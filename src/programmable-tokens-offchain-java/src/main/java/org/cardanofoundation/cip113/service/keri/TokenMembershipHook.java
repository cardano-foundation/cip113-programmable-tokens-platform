...
/** Per-module hook fired after a KYC attestation is generated for a session
 *  bound to a token policy. Lets each module plug its own onboarding side
 *  effect (e.g. inserting into its allowlist) without {@code KeriService} having
 *  to know about any module-specific service or repo. */
public interface TokenMembershipHook {

    /** Module id this hook handles (e.g. {@code "kyc-extended"},
     ...
     *  per-module side effects (allowlist upsert, audit log, etc.).
