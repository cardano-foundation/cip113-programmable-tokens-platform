...
public class RwaTokenAllowlistService {
    ...
    // (POST /rwa-token/{policyId}/update-member-root-hash).
    ...
    public TrieSnapshot snapshotForPublish(String policyId) {
        List<RwaTokenMemberLeafEntity> leaves = leafRepo.findByProgrammableTokenPolicyId(policyId);
        MpfTrie trie = buildTrieFromLeaves(leaves, securityPolicyIdOf(policyId), networkId());
        java.util.Set<Long> ids = leaves.stream()
                .map(RwaTokenMemberLeafEntity::getId)
    ...
    public int markLeavesPublishedById(java.util.Set<Long> leafIds) {
        ...
        int count = 0;
        for (RwaTokenMemberLeafEntity leaf : leafRepo.findAllById(leafIds)) {
    ...
    public PruneResult pruneExpired(String policyId, long cutoffMs) {
        List<RwaTokenMemberLeafEntity> expired = leafRepo.findExpired(policyId, cutoffMs);
        if (expired.isEmpty()) {
            return new PruneResult(currentRoot(policyId), 0);
        }
        leafRepo.deleteAllInBatch(expired);
        List<RwaTokenMemberLeafEntity> all = leafRepo.findByProgrammableTokenPolicyId(policyId);
        java.util.Set<Long> deletedIds = expired.stream()
                .map(RwaTokenMemberLeafEntity::getId)
                .collect(java.util.stream.Collectors.toSet());
        List<RwaTokenMemberLeafEntity> surviving = all.stream()
    ...
    private MpfTrie buildTrieFromLeaves(List<RwaTokenMemberLeafEntity> leaves,
                                        byte[] securityPolicyId, int networkId) {
        TestNodeStore store = new TestNodeStore();
        MpfTrie trie = new MpfTrie(store);
        for (RwaTokenMemberLeafEntity leaf : leaves) {
            trie.put(
