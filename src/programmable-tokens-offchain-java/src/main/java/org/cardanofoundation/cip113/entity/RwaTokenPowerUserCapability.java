...
/** Capabilities a power user can hold for a registered rwa-token.
...
public enum RwaTokenPowerUserCapability {
    ...
    RwaTokenPowerUserCapability(int bit) {
        this.bit = bit;
    }
    ...
    public static int encode(RwaTokenPowerUserCapability... caps) {
        int out = 0;
        for (RwaTokenPowerUserCapability c : caps) out |= c.bit;
        return out;
    }
}
