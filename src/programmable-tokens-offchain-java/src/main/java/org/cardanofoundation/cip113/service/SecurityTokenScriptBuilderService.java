...
public class SecurityTokenScriptBuilderService {
    ...
    /** Every parameterised security-token script for one token, plus the derived
    ...
    public record SecurityTokenScripts(
    ...
    private static String hashHex(PlutusScript script) {
        try {
            ...
            throw new IllegalStateException("could not hash a parameterised security-token script", e);
        }
    }
    ...
    private static String policyId(PlutusScript script) {
        try {
            ...
            throw new IllegalStateException("could not derive the policy id of a parameterised security-token script", e);
        }
    }
     ...
     */
    public SecurityTokenScripts resolveScripts(String securityAssetNameHex,
        ...
        String plbScriptHash = protocolParams.programmableLogicBaseParams().scriptHash();
        ...
        PlutusScript transferLogic = buildTransferLogicScript(
                securityAssetNameHex, globalStatePolicyId, registryPolicyId,
                expectedIssuancePolicyId, denylistScriptHash, plbScriptHash);

        PlutusScript thirdPartyTransferLogic = buildThirdPartyTransferLogicScript(
                securityAssetNameHex, powerUsersPolicyId, globalStatePolicyId, registryPolicyId,
                expectedIssuancePolicyId, denylistScriptHash, powerUserListScriptHash, plbScriptHash);
        ...
        PlutusScript mintingAuthority = buildMintingAuthorityScript(
                ...
                expectedIssuancePolicyId, plbScriptHash, denylistScriptHash, powerUserListScriptHash,
        ...
        return new SecurityTokenScripts(
    ...
    public PlutusScript buildMintingAuthorityScript(String securityAssetNameHex,
        ...
        ListPlutusData params = ListPlutusData.of(
                ...
                BytesPlutusData.of(HexUtil.decodeHexString(plbScriptHash)),
     ...
     *  @param plbScriptHash the CIP-113 {@code programmable_logic_base}, the payment
     *  credential every token UTxO must keep. */
     ...
     *
     * <p>Parameter order mirrors {@code third_party_transfer_logic_script.ak}: note that
     * {@code power_users_linked_list_policy_id} comes <em>before</em>
     * {@code global_state_policy_id} here, unlike in {@link #buildMintingAuthorityScript}.
     * Both are 28-byte hex, so swapping them compiles and yields a wrong hash silently.
     */
